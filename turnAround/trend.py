...
Для нисходящего тренда следующие требования:ниже 8 средней, берем 7 последних свечей, любые последовательные 4 из них должны 
быть под 8 и их хай должен уменьшаться
...
def trendDown(df, pathList, i):
    try:
        ...
        trendNumber = df[-7:].loc[df.Close < df.ema8]  # берем 10 дней, хаи ниже 20 средней
        if trendNumber.shape[0] >= 4:
            count = 0
            firstIndex = 1
