...
def mplot(df, ticker, folder, lines):
    ...
    fig, axes = mpf.plot(df, type='candle', style='yahoo', title=ticker, volume=True, mav=(8, 13, 21, 55),
                         ...
                         hlines=dict(hlines=lines[0][1], colors='k', linewidths=0.8))
...
def serching(folder, text, patternName):
    ...
    for i in ticks:
        ...
        if findWaves(df):
            ...
            # статистика
            stat.addStat(
                [str(i)[:-4], patternName, str(folder).replace('/home/linac/Рабочий стол/data/', ''), 'noTrend',
                 date.today(), float(df.Close[-1:]), float(df.Close[-1:]) * 1.02, float(df.Close[-1:]) * 0.98,
                 float(df.Close[-1:])])
            # передаем в ib 15 бумаг
            if (count < 30):
                try:
                    ib2.starHere(str(i)[:-4], round(float(df.Close[-1:]), 2), round((float(df.Close[-1:]) * 1.02), 2),
                                 round((float(df.Close[-1:]) * 0.98), 2))
                except:
                    pass
            count += 1
...
name = 'test'
patternName = 'WW'#
folder1 = '/home/linac/Рабочий стол/data/20210612_60d1d'
